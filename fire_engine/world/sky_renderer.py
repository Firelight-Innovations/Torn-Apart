...
Draws everything the headless ``fire_engine.sky`` simulation describes through
...
``fire_engine.world.__init__`` exports ``SkyRendererComponent`` behind the same
...
from fire_engine.world import sky_shaders
...
class SkyRendererComponent(Component):
    ...
    * ``late_update(dt)`` — reads the frame's ``SkyState`` and writes the
    ...
    sky_system : fire_engine.sky.SkySystem (or duck-typed stub)
    ...
    def _build_dome(self, star_count: int) -> None:
        ...
        from fire_engine.world.texture_bridge import to_panda_cubemap
    ...
    def _build_clouds(self) -> None:
        ...
        from fire_engine.world.texture_bridge import to_panda_texture_3d
