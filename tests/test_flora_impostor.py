...
# ---------------------------------------------------------------------------
BARK_PALETTE = np.array([(40, 31, 22), (58, 46, 33), (79, 63, 45)],
                        dtype=np.uint8)
...
class TestOutputShapeAndDtype:
    def test_default_cell_shape(self):
        ...
        cell = _default_cell()
        assert cell.shape == (96, 64, 4), (
            f"Expected (96, 64, 4), got {cell.shape}"
        )
    ...
    def test_custom_cell_shape_square(self):
        ...
        rng = _make_rng("sq")
        sk, leaves = _grow_oak(rng)
        cell = rasterize_impostor(sk, leaves, BARK_PALETTE, LEAF_PALETTE, rng,
                                  cell_wh=(32, 32))
...
class TestBinaryAlpha:
    def test_alpha_only_0_or_255(self):
        ...
        assert set(unique.tolist()).issubset({0, 255}), (
            f"Non-binary alpha values found: {unique}"
        )
...
class TestLeaflessSkeletonTrunkPaint:
    ...
    def _bare_cell(self, cell_wh=(64, 96)):
        ...
        return rasterize_impostor(sk, empty_leaves,
                                  BARK_PALETTE, LEAF_PALETTE, rng2,
                                  cell_wh=cell_wh)
...
class TestPxPerMScaling:
    ...
    def _render(self, px_per_m, cell_wh=(64, 96)):
        ...
        sk, leaves = _grow_oak(rng)
        return rasterize_impostor(sk, leaves, BARK_PALETTE, LEAF_PALETTE, rng,
                                  cell_wh=cell_wh, px_per_m=px_per_m)
    ...
    def test_larger_px_per_m_more_opaque_pixels(self):
        ...
        cell_small = rasterize_impostor(sk_s, leaves_s, BARK_PALETTE,
                                        LEAF_PALETTE, rng_s,
                                        cell_wh=(64, 96),
        ...
        cell_large = rasterize_impostor(sk_l, leaves_l, BARK_PALETTE,
                                        LEAF_PALETTE, rng_l,
                                        cell_wh=(64, 96),
