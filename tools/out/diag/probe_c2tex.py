...
def readback(tex):
    ...
    if ct == Texture.T_float:
        arr = np.frombuffer(
            memoryview(pta), dtype=np.float16 if tex.get_component_width() == 2 else np.float32
        )
    else:
...
for casc in pipe.cascades:
    ...
    if rad is not None:
        ...
        if 0 <= kz < casc.cells:
            ...
            sl_rad = mag[kz]
            print(
                f"  z=8m slice (k={kz}): sun vis mean={sl_sun.mean():.3f} rad mean={sl_rad.mean():.4f}"
            )
