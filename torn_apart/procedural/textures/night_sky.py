...
class NightSkyDef(ProceduralTextureDef):
    ...
    def generate(self, rng: np.random.Generator, **params) -> np.ndarray:
        ...
        **params : any
        ...
        filaments = _seamless_u(value_noise(
            rng, (H, W), octaves=5, persistence=0.55, lacunarity=2.2, base_freq=8,
        ))
        # Sharpen hard: push midtones way down so bright threads pop.
        filaments = filaments ** 2.4
        halo_mottle = _seamless_u(value_noise(
            rng, (H, W), octaves=4, persistence=0.5, lacunarity=2.0, base_freq=4,
        ))
        dust = _seamless_u(value_noise(
            rng, (H, W), octaves=4, persistence=0.5, lacunarity=2.0, base_freq=9,
        ))
        ...
        galaxy = core * (0.35 + 1.30 * filaments) \
            + 0.45 * halo * (0.25 + 0.75 * halo_mottle)
        galaxy *= 1.0 - 0.90 * dust_lanes * np.sqrt(
            np.clip(core + 0.4 * halo, 0.0, 1.0)
        )
        # Fine grain so saturated core regions keep visible texture.
        fine = _seamless_u(value_noise(
            rng, (H, W), octaves=3, persistence=0.6, lacunarity=2.0, base_freq=24,
        ))
        ...
        neb_teal = _seamless_u(value_noise(
            rng, (H, W), octaves=3, persistence=0.6, lacunarity=2.0, base_freq=2,
        ))
        neb_violet = _seamless_u(value_noise(
            rng, (H, W), octaves=3, persistence=0.6, lacunarity=2.0, base_freq=3,
        ))
        ...
        in_plane = (
            np.cos(alpha)[:, None] * e1[None, :]
            + np.sin(alpha)[:, None] * e2[None, :]
        )
        d_band = (
        ...
        rows = np.clip(
            ((1.0 - (lat_all / np.pi + 0.5)) * H).astype(np.int64), 0, H - 1
        )
        ...
        for dr, dc, wgt in (
            ...
            (-2, 0, 0.15), (2, 0, 0.15), (0, -2, 0.15), (0, 2, 0.15),
        ):
...
def _dirs_to_face_pixels(dirs: np.ndarray, size: int
    ...
    fy = ~fx & (ay >= az)
    fz = ~fx & ~fy
    face = np.where(fx, np.where(x >= 0, 0, 1),
                    np.where(fy, np.where(y >= 0, 2, 3),
                             np.where(z >= 0, 4, 5)))
    ma = np.where(fx, ax, np.where(fy, ay, az))
    # GL per-face (sc, tc) selection table.
    sc = np.where(face == 0, -z, np.where(face == 1, z,
                  np.where(face == 5, -x, x)))
...
def _hash3i(ix: np.ndarray, iy: np.ndarray, iz: np.ndarray,
            seed: int) -> np.ndarray:
    """Vectorised integer-lattice hash → float32 in [0, 1)."""
    h = (ix.astype(np.int64) * 73856093
         ^ iy.astype(np.int64) * 19349663
         ^ iz.astype(np.int64) * 83492791
         ^ np.int64(seed)).astype(np.uint32)
    h ^= h >> np.uint32(15)
...
def _value_noise_3d(points: np.ndarray, base_freq: float, octaves: int,
                    seed: int, persistence: float = 0.55,
                    lacunarity: float = 2.1) -> np.ndarray:
    """
...
class NightSkyCubeDef(ProceduralTextureDef):
    ...
    def generate(self, rng: np.random.Generator, **params) -> np.ndarray:
        ...
        **params : any
        ...
        n = np.array([sin_i * math.cos(band_az),
                      sin_i * math.sin(band_az), cos_i], dtype=np.float32)
        ...
        filaments = _upsample2_faces(
            _value_noise_3d(half, 7.0, 5, nseed + 1)) ** 2.4
        halo_mottle = _upsample2_faces(
            _value_noise_3d(half, 3.5, 4, nseed + 2, persistence=0.5))
        dust = _upsample2_faces(
            _value_noise_3d(half, 8.0, 4, nseed + 3, persistence=0.5))
        fine = _upsample2_faces(
            _value_noise_3d(half, 21.0, 3, nseed + 4, persistence=0.6))
        dust_lanes = np.clip((dust - 0.45) / 0.28, 0.0, 1.0)
        dust_lanes = dust_lanes * dust_lanes * (3.0 - 2.0 * dust_lanes)
        ...
        rgb[:] = _SKY_FLOOR
        ramp = _ramp_rgb(galaxy.reshape(6 * S, S),
                         _GALAXY_RAMP_KEYS, _GALAXY_RAMP_RGB
                         ).reshape(6, S, S, 3)
        rgb += ramp * galaxy[..., None]

        # --- Nebula tints ---------------------------------------------------
        neb_teal = _upsample2_faces(
            _value_noise_3d(half, 1.8, 3, nseed + 5, persistence=0.6))
        neb_violet = _upsample2_faces(
            _value_noise_3d(half, 2.6, 3, nseed + 6, persistence=0.6))
        rgb += np.array([0.025, 0.075, 0.090], np.float32) \
            * np.clip((neb_teal - 0.55) / 0.35, 0.0, 1.0)[..., None]
        rgb += np.array([0.085, 0.035, 0.110], np.float32) \
        ...
        d_uni = np.stack([r_u * np.cos(phi_u), r_u * np.sin(phi_u), z_u],
                         axis=1)

        e1 = np.array([-math.sin(band_az), math.cos(band_az), 0.0])
        e2 = np.cross(n, e1)
        alpha = rng.uniform(0.0, 2.0 * np.pi, n_band)
        beta = np.clip(rng.normal(0.0, _BAND_STAR_SIGMA_RAD, n_band),
                       -0.45, 0.45)
        in_plane = (np.cos(alpha)[:, None] * e1[None, :]
                    + np.sin(alpha)[:, None] * e2[None, :])
        d_band = (np.cos(beta)[:, None] * in_plane
                  + np.sin(beta)[:, None] * n[None, :])
        ...
        b_rgb = star_rgb[bright_mask]
        for dr, dc, wgt in ((-1, 0, 0.30), (1, 0, 0.30),
                            (0, -1, 0.30), (0, 1, 0.30)):
