# zones — System Doc
keywords: zone, volume, region, grass, vegetation, foliage, blades, tuft, instancing, gl_InstanceID, biome, spawn area, box volume, height field, placement, density, ZoneVolume, ZoneStore, trees, leaves, leaf litter, wind particles, flora, flowers, flower, bushes, bush, shrub, scrub, flora_placement, flora_hash_seed, flora_instance_count, flora_instance_attribs, FLORA_KINDS, atlas variant, tree_placement, TREE_KINDS, TreeInstances, bake_tree_instances, instances_data_block, species_mix_from_params, SCALE_JITTER, species, species_mix, jittered grid, min spacing, data texture, variant, 3D trees

## Role
Tagged axis-aligned box volumes in world space, and the math behind systems that act on them. A `ZoneVolume` with `tag="grass"` tells the GPU grass renderer (`world/grass_renderer.py`) where blades grow; `tag="flowers"` volumes tell the flora renderer (`world/flora_renderer.py::FloraRendererComponent`) where wildflower sprites stand; `tag="trees"` / `tag="bushes"` volumes tell the 3-D tree renderer (`world/tree_renderer.py::TreeRendererComponent`) where instanced tree/bush meshes grow (`tree_placement.py` bakes their placements on the CPU); `tag="trees"` volumes ALSO tell the wind system's leaf-litter renderer (`world/mote_renderer.py::LeafLitterComponent`) where fallen leaves scatter and stream on gusts (one forest volume → trees + their litter); `tag="biome"` volumes are reserved for surface-material regions (snow / bare dirt — planned, not yet consumed). The `ZoneStore` registry holds all volumes and participates in delta saves (`save_key="zones"`). `grass_placement.py` and `flora_placement.py` are the headless, testable half of the GPU-instanced vegetation (python mirrors of the shaders' instance hash chains, per-volume instance counts, the height-field bake); `tree_placement.py` is CPU-authoritative placement for trees (small counts, guaranteed spacing, game-knowable positions). This package deliberately does NOT touch panda3d or render anything.
...
- `FLORA_KINDS` — `("flowers",)`, the zone tags the sprite-flora renderer consumes (`flora_placement.py`; bushes/trees graduated to `tree_placement.py`).
- `flora_hash_seed(volume, kind)` — per-(volume, kind) shader seed via `for_domain("zones", "flora", kind, volume.id)`.
...
- `TREE_KINDS` — `("trees", "bushes")`, the zone tags the 3-D tree renderer consumes (`tree_placement.py`).
- `SCALE_JITTER` — per-kind `(scale_min, scale_span)` instance size jitter; public because the renderer pads culling bounds by max scale.
- `species_mix_from_params(params, default)` — resolves a volume's species mix: `params["species"]="tree_dead"` or `params["species_mix"]="tree_gnarled_oak:3,tree_dead:1"` (weighted `name:weight` pairs) → `[(name, weight), ...]`.
- `bake_tree_instances(volume, config, chunks, species_weights, variant_counts, kind)` — CPU-baked `TreeInstances` for one volume: **jittered grid** (cell `c = max(min_spacing, 1/√density)`, jitter ±0.35·c, keep-prob `density·c²` → any two trees ≥ 0.3·c apart), Z decoded from the grass height-field bake (sentinel = drop), deterministic via `for_domain("zones", "tree_placement", kind, volume.id)`, capped at `config.<kind>_max_instances`. Per-instance: position, yaw, scale, sway phase, tint, species index (weighted), mesh-pool variant.
- `TreeInstances` — the bake result SoA (`x/y/z/yaw/scale/phase/tint` float32, `species_idx/variant` int32, `species_names`, `count`).
- `instances_data_block(inst, mask=None)` — packs (optionally masked) instances into the renderer's `(n, 2, 4) float32` **data-texture block**: texel `(0,i)`=`(x,y,z,yaw)`, texel `(1,i)`=`(scale,phase,tint,variant)`. The GLSL `texelFetch` contract with `world/shaders/tree.vert` / `tree_impostor.vert` — `tests/test_tree_placement.py` pins it.
...
## The `"trees"` tag (3-D trees + wind leaf litter — one volume, two consumers)
