...
> One doc per code package; filename matches the package exactly (`docs/systems/simulation.player.md` ↔ `fire_engine/simulation/player/`).
...
All symbols below are re-exported from `fire_engine.simulation.player` (`__init__.py`).
...
`player/` may import:
- `fire_engine.render` (Component, Transform, etc.)
