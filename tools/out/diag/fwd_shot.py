...
import os
FLY=int(sys.argv[1]); NAME=sys.argv[2]; TOD=float(sys.argv[3])
app=demo.build_demo()
app.input_state.mouse_captured=False; app._set_mouse_capture(False)
app._clock.game_time_of_day=TOD*3600.0; hold=app._clock.game_time_of_day
...
img=PNMImage(); app.win.get_screenshot(img)
out=_R/"tools"/"out"/"diag"/NAME; img.write(Filename.from_os_specific(str(out)))
w,h=img.get_x_size(),img.get_y_size(); s=0;n=0
for yy in range(int(h*0.55),h,4):
    for xx in range(0,w,8): s+=img.get_bright(xx,yy);n+=1
print(NAME,"ground-region mean bright=%.3f"%(s/n),"exposure=%.2f"%app.lighting_pipeline.exposure)
