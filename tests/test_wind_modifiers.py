...
class TestWindModifierProtocol:
    ...
    def test_protocol_is_runtime_checkable(self):
        ...
        import typing
        assert hasattr(WindModifier, "__protocol_attrs__") or \
               getattr(WindModifier, "_is_protocol", False) or \
               isinstance(_front(), WindModifier)  # structural proof
...
class TestBandTravel:
    def test_max_delta_shifts_with_time(self):
        ...
        front = _front(seed_key=("travel",), direction=(1.0, 0.0),
                       speed=12.0, width_m=20.0)
        t = 5.0
...
class TestParameterScaling:
    def test_larger_strength_larger_delta(self):
        ...
        assert vx_hi.max() > vx_lo.max(), \
            "Higher strength did not produce larger peak vx"

    def test_strength_monotone_scaling(self):
        """Increasing strength strictly increases the maximum vx delta."""
        X, Y = np.meshgrid(np.linspace(0, 128, 33), np.linspace(0, 128, 33),
                           indexing="ij")
...
class TestEdgeCases:
    def test_zero_strength_no_change(self):
        ...
        X, Y, vx, vy, turb = _make_grid()
        vx[:] = 3.0; vy[:] = 1.5; turb[:] = 0.5
        vx_before = vx.copy(); vy_before = vy.copy(); turb_before = turb.copy()
