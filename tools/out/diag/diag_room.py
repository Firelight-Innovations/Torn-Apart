...
print("c0", cfg.light_c0_cells, cfg.light_c0_cell_m,
      "c1", cfg.light_c1_cells, cfg.light_c1_cell_m,
      "c2", cfg.light_c2_cells, cfg.light_c2_cell_m)

# Build the room geometry into a chunk dict (mirror build_gi_test_room geometry).
chunks = {}
def provider(coord):
    if coord not in chunks:
        chunks[coord] = Chunk(coord, chunk_size=cfg.chunk_size,
...
def box(half, at, mode, material=1):
    apply_brush(BoxBrush(half_extents_m=Vec3(*half)), Vec3(*at), mode,
                material=material, chunk_provider=provider, bus=bus)
...
box((0.75,1.25,1.5),(cx-5.0,cy,z0+1.5),BrushMode.REMOVE)  # doorway (along_x False default? camera fwd +Y)
...
def report(cells, cell_m, name, campos):
    ...
    win.recenter(campos)
    vol = assemble_geometry(win, chunks, pal, chunk_size=cfg.chunk_size,
                            voxel_size=cfg.voxel_size)
    occ = vol.albedo_occ[...,3]
    ox,oy,oz = win.origin_cell
    # interior sample points (world m) inside the empty room volume
    pts = [(cx,cy,cz),(cx,cy-2,cz),(cx,cy+2,cz),(cx-2,cy,cz),(cx+2,cy,cz),
           (cx,cy,cz-1.5),(cx,cy,cz+1.5)]
