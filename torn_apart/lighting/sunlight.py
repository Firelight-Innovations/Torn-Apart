...
1. **Occupancy** — for each chunk, call
   :func:`torn_apart.lighting.light_grid.occupancy_from_materials` to produce a
...
def make_light_sampler(
    ...
    >>> from torn_apart.lighting import LightGrid, make_light_sampler
    >>> lg = LightGrid()
...
class SunlightComputer:
    ...
    >>> from torn_apart.lighting import LightGrid, SunlightComputer
    >>> set_world_seed(1337)
    >>> cfg = load_config()
    >>> bus = EventBus()
