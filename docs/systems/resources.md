...
> One doc per code package; filename matches the package exactly (`docs/systems/resources.md` ↔ `torn_apart/resources/`).
...
All symbols below are re-exported from `torn_apart.resources` (`__init__.py`).
...
- `torn_apart.core` (logging etc. if needed — currently not imported)
...
Procedural environment textures generated by `torn_apart.procedural` (e.g. `wasteland_ground`) — those are numpy arrays bridged to panda3d via `world/texture_bridge.py`. Only **hand-crafted** files on disk go through `ResourceManager`.
...
```python
from torn_apart.resources import load, acquire, release, unload_unreferenced
