...
cam.local_rotation = (Quat.from_axis_angle(Vec3.UP, math.radians(180.0))
                      * Quat.from_axis_angle(Vec3.RIGHT, math.radians(-6.0))).normalized()
for _ in range(8):
    app.taskMgr.step(); app._clock.game_time_of_day = hold

for casc in pipe.cascades:
    om=casc.window.world_origin_m; sz=casc.window.size_m
...
ks=list(app.chunk_manager.chunks.keys()); print("chunk y range", min(k[1] for k in ks), max(k[1] for k in ks))
