...
:func:`~fire_engine.wind.venturi.solve_venturi` for the same inputs (asserted in
...
class VenturiWorker:
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._run, name="WindVenturiWorker", daemon=True)
        self._thread.start()
    ...
    def _run(self) -> None:
        while True:
            ...
            try:
                ...
                _log.exception(
                    "Venturi solve failed (origin %r, seq %d)",
                    job.origin_cell, job.seq)
