...
(:mod:`fire_engine.weather.system`) produces a *continuous* local state —
...
* is **consistent between the weather-map raster and a local sample** by
  ...
  caller is :class:`~fire_engine.weather.WeatherMap` over a grid or
  :meth:`~fire_engine.weather.WeatherSystem.sample_local` at one point;
...
class CloudGenus(str, Enum):
    ...
    :class:`~fire_engine.weather.CellKind` / :class:`WeatherType` convention).
