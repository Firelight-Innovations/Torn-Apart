...
class WeatherSystem:
    ...
    def __init__(self, config: Config, bus: EventBus | None = None) -> None:
        ...
        self._wind_field = None  # fire_engine.world.wind.WindField | None
    ...
    def summon_cell(
        ...
        *toward* the player on the steering current (``cell.center`` rides the
        raw synoptic displacement — see :mod:`fire_engine.world.weather.cells`).  It is
    ...
    def attach_wind_field(self, wind_field: Any) -> None:
        ...
        weather system and the :class:`~fire_engine.world.wind.WindField`).  ``None``
    ...
    def _update_gust_fronts(self, t: float, player_pos: tuple[float, float]) -> None:
        ...
        :class:`~fire_engine.world.wind.GustFront` is registered on the attached wind
    ...
    def _emergent_fog(
        ...
        Closed-form condensation (see :mod:`fire_engine.world.weather.humidity`): the
    ...
    def sample_fields(
        ...
        :mod:`fire_engine.world.weather.humidity`), so calm humid nights grow ground
    ...
    def rain_recent_at(self, points_xy: np.ndarray, t_abs: float) -> np.ndarray:
        ...
        (:func:`fire_engine.world.weather.humidity.relative_humidity`).  Pure function
    ...
    def _emit_lightning(self, abs_t: float, cells: list[StormCell]) -> None:
        ...
        :mod:`fire_engine.world.weather.lightning`), so this never double-emits across
    ...
    def update(
        ...
        # nearby cells (no-op when no wind field is attached). ---
        self._update_gust_fronts(abs_t, pos)

        # --- M7 lightning emission ---------------------------------------
        # Emit one LightningStrikeEvent per scheduled strike in (last, now] for
        # every active THUNDERSTORM cell.  No-op when there is no bus.
        # See weather/lightning.py.
