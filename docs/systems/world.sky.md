...
All symbols below are re-exported from `fire_engine.world.sky` (`__init__.py`).
...
### Weather (now `fire_engine/world/weather/`; `sky/weather.py` is a re-export shim)
...
imports keep working; new code imports from `fire_engine.world.weather`.
...
**No panda3d imports.** Never import from `world/`, `terrain/`, `lighting/`, or any higher layer.  The render half (`world/`) imports `fire_engine.world.sky` downward — never the reverse.
...
- **Override blending:** `force_weather` (the dev shim) crossfades over `BLEND_SECONDS` = 20 game minutes with smoothstep — params never pop.  Per-state targets: CLEAR(coverage .12, density .35, fog .0008, rain 0) · CLOUDY(.45, .55, .0012, 0) · OVERCAST(.85, .80, .003, 0) · FOG(.55, .50, .025, 0, calm ×.30) · RAIN(.90, .85, .006, .7) · STORM(.98, .95, .008, 1.0, gusty ×1.9).  Wind comes from the closed-form **synoptic flow** (`fire_engine.world.weather.Synoptic`, exposed as `WeatherSystem.synoptic`): direction drifts smoothly over hours, speed = synoptic speed × the per-state multiplier above.  See `docs/systems/weather.md`.
...
```python
from fire_engine.world.sky import sun_direction, moon_direction
