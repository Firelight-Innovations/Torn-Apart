...
class DeadTreeDef(TreeSpeciesDef):
    ...
    BARK_PALETTE = np.array([(42, 38, 32), (62, 57, 48), (84, 78, 66)], dtype=np.uint8)
    # The few surviving tufts read dry and dusty.
    LEAF_PALETTE = np.array(
        [(52, 48, 30), (72, 66, 40), (94, 86, 52), (112, 102, 64)], dtype=np.uint8
    )
    ...
    def grow(self, rng: np.random.Generator, variant: int) -> tuple[TreeSkeleton, Leaves]:
        ...
        ids = np.concatenate([limbs, twigs]) if twigs.size or limbs.size else trunk
        tips = sk.tip_ids(ids)
