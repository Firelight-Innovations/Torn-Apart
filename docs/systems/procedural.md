...
| `Leaves`, `leaves_at_tips(sk, ids, rng, density, leaf_size_m, leaves_per_m, max_leaves, sway_min, max_offset_m, ...)` | `flora/leaves.py` | INDIVIDUAL leaves anchored ALONG the leaf-bearing segments in `ids` (biased to thinner/outer twigs): count `= round(density·leaves_per_m·Σ segment_length)`, capped at `max_leaves`; each leaf sits within `≈radius + 1.5·leaf_size` of its host so it never floats.  More/finer twigs ⇒ denser crown.  (`cell_m`/`rounds`/`per_cell` accepted but legacy/ignored.) |
