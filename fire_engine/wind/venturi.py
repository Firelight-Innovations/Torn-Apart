...
if TYPE_CHECKING:  # avoid an import cycle (worker imports venturi)
    from fire_engine.wind.worker import VenturiJob, VenturiResult
...
def column_solid_fraction(job: "VenturiJob") -> np.ndarray:
    ...
    convention as :class:`~fire_engine.wind.region.WindRegion`), so the returned
...
def solve_venturi(job: "VenturiJob") -> "VenturiResult":
    ...
    Returns a :class:`~fire_engine.wind.worker.VenturiResult` carrying a
    ...
    ----------
    job : VenturiJob
        The solve request (see :class:`~fire_engine.wind.worker.VenturiJob`).
    ...
    from fire_engine.wind.worker import VenturiResult
