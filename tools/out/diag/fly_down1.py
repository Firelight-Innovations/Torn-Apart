...
for i in range(600):
    y+=1.0; cam.position=Vec3(0.0,y,10.0)
...
img=PNMImage(); app.win.get_screenshot(img); img.write(Filename.from_os_specific(str(out)))
w,h=img.get_x_size(),img.get_y_size(); s=0;n=0
for yy in range(int(h*0.6),h,5):
    for xx in range(0,w,10): s+=img.get_bright(xx,yy);n+=1
