...
print(
    "  -> base*(direct+radiance): GI adds %.1f%% on top of direct"
    % (100 * gi_ground[1] / (sun[1] * 0.93))
)
print(
    "  (this 'GI' is essentially just skylight ambient ~%.2f; no colored bounce on flat open ground)"
    % sky[1]
)
...
print(
    "  first-bounce injected into adjacent air = (%.3f,%.3f,%.3f) lam=%.2f"
    % (firstbounce[0], firstbounce[1], firstbounce[2], lam)
)
print(
    "  note: if wall faces away from sun (interior, sun blocked) lam may be ~0 -> NO bounce injected."
)
