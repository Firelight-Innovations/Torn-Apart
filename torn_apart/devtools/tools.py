...
its current :class:`~torn_apart.devtools.fields.Panel`, and exposes a ``revision``
...
registering an instance with :class:`~torn_apart.devtools.manager.DevToolsManager`
...
from torn_apart.devtools.fields import Field, FieldKind, Section, Button, Panel
...
class InspectorTool(DevTool):
    ...
    :class:`~torn_apart.devtools.selection.Selection`) whenever the selection
    changes; between changes it shows live values and applies edits straight to
    the object through :func:`~torn_apart.devtools.introspect.describe_object`.
