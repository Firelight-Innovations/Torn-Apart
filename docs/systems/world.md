...
GPU-only instanced flora — flowers, bushes and trees — for every `tag="flowers"` / `tag="bushes"` / `tag="trees"` `ZoneVolume`: the grass idiom generalised over a per-kind spec table (headless math in `zones/flora_placement.py` — see `docs/systems/zones.md`). **GPU lighting backend only** (logs and disables itself on the CPU backend, like grass).
...
Per kind it binds a seeded procedural sprite **atlas** (`"flower_sprite"` 4 cells / `"bush_sprite"` 3 / `"tree_sprite"` 3) on a shared crossed-quad `Geom` (2 quads for flowers, 3 for bushes/trees; tree quads keep the atlas cell's 2:3 aspect so texels stay square) and a sway shape: `u_sway_gain` scales amplitude vs grass (flowers 0.8, bushes 0.3, trees 0.15) and `u_sway_pivot` sets the normalised height where bending starts (0 = whole-plant blade bend; 0.45 pins a tree trunk so only the canopy sways). `flora.vert` is the grass hash chain **plus one link** — `h5` selects the atlas variant — mirrored line-for-line by `zones/flora_placement.py::flora_instance_attribs` (edit both or neither; `tests/test_flora.py` pins the python side and the chain constants). Wind is the same dual path as grass: per-plant `u_wind_tex` sampling when the wind field is live, the scalar SkyState sway fallback otherwise — one gust band rolls through meadow, scrub and treeline together. Height fields (`bake_grass_height_field` — generic over any volume), bounds discipline (`BoundingBox` + `set_final`), shader-on-the-instanced-node (gotcha 18's mirror discipline applies to `flora.vert` too), per-volume re-bakes on `TerrainEditedEvent`/`ChunkLoadedEvent`, and cascade/fog lighting by `terrain_root` inheritance are all exactly the grass component's; `u_time_s` is self-bound on `flora_root` (not inherited — the wind-particles gotcha). A `"trees"` volume is shared infrastructure: this component draws the tree sprites while the wind system's `LeafLitterComponent` independently scatters leaf litter over the same box. Config: the `[flora]` table (`flora_<kind>_density_per_m2` / `_height_m` / `_fade_start_m` / `_fade_end_m` / `_max_instances`); per-volume `params["density"]` overrides the density. The fragment shader is grass's lighting contract with a per-kind `u_light_offset_m` cascade sample height (0.5 m flower → 3 m tree canopy).
...
19. **The procedural ground uses per-octave LOD, NOT a collapse-to-mean.** `terrain.frag`'s `groundNoise` sums three hash octaves (fine 1×, mid 4×, macro 16× larger texels); each octave individually fades toward the hash mean (0.5) via `smoothstep(0.5, 1.4, mpp*texels)` once a screen pixel approaches the size of *that octave's* texels (`mpp` = world metres/pixel from `fwidth`).  So the fine detail drops out first, then the mid, leaving the macro colour patches — distant ground stays varied instead of flattening to one green.  The earlier single global `mix(gnoise, 0.5, …)` collapsed *all* scales together and produced the "sea of green after a few metres" — do not reintroduce it.  Each octave has mean 0.5, so the weighted sum's mean stays 0.5 and the posterise buckets stay balanced.  `ground_texels_per_m` sets the fine octave; raise it for crisper near pixels (the LOD prevents the distant shimmer that used to force it down).

20. **`terrain.frag` samples THREE radiance cascades, finest-first.** `sampleCascades` tries cascade 0 (0.5 m), then 1 (2 m), then 2 (8 m, the coarse far cascade), then the open-sky guess.  The `u_c2_*` uniform set (geom/vis/radiance/origin/cell_m/cells) must stay in sync with `GpuLightingPipeline.bind_surface_inputs`/`update_surface_inputs` — adding a cascade means a matching block in both the shader and those two methods.  The grass shader (`grass.frag`) inherits these uniforms by parenting under `terrain_root`, so it gets the far cascade for free.

21. **Posterise per supersample tap, then average the COLOURS — never average the noise and posterise once.** The ground palette LUT (`u_ground_lut`) is a hard quantiser: feeding it one averaged `groundNoise` value re-hardens everything the supersample + octave LOD smoothed, because the averaged value hovers near a palette-bucket edge and pops a whole palette step on any sub-pixel camera move.  This ordering bug — not the normal map and not the light-quant grid, both measured innocent — was the dominant terrain "z-fighting"/shimmer (`tools/shimmer_probe.py`: far-ground flip fraction 0.0080 → 0.0003 after reordering, ~27× less; see `tools/out/diag/probe_baseline` vs `probe_fix_postlut`).  `terrain.frag` therefore runs the LUT lookup inside the 4-tap loop and averages the resulting colours.  Up close all 4 taps land in one texel → one palette colour → pixel art stays crisp.  Any future quantiser (palette, posterise, dither) in this shader must follow the same rule: filter *after* quantisation.
