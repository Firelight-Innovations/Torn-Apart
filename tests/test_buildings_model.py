...
def _building(name: str = "test") -> Building:
    return Building(name=name, position=Vec3(0.0, 0.0, 8.0),
...
class TestElementIds:
    def test_ids_are_monotonic_and_unique(self):
        ...
        o = s.add_opening(w1.id, OpeningKind.DOOR, offset_m=1.0,
                          width_m=0.9, head_m=2.0)
...
class TestOpenings:
    def test_door_defaults_to_floor_level(self):
        ...
        s = b.add_storey()
        w = s.add_wall((0, 0), (4, 0))
        o = s.add_opening(w.id, OpeningKind.DOOR, offset_m=1.0,
                          width_m=0.9, head_m=2.0)
        assert o.sill_m == 0.0
        assert w.openings == [o]
    ...
    def test_opening_beyond_wall_length_rejected(self):
        ...
        with pytest.raises(ValueError):
            s.add_opening(w.id, OpeningKind.WINDOW, offset_m=1.5,
                          width_m=1.0, sill_m=1.0, head_m=2.0)
    ...
    def test_opening_above_wall_band_rejected(self):
        ...
        w = s.add_wall((0, 0), (4, 0))
        with pytest.raises(ValueError):
            s.add_opening(w.id, OpeningKind.WINDOW, offset_m=1.0,
                          width_m=1.0, sill_m=1.0, head_m=2.9)
    ...
    def test_sill_must_be_below_head(self):
        ...
        with pytest.raises(ValueError):
            s.add_opening(w.id, OpeningKind.WINDOW, offset_m=1.0,
                          width_m=1.0, sill_m=2.0, head_m=2.0)

    def test_unknown_wall_id_rejected(self):
        b = _building()
        s = b.add_storey()
        with pytest.raises(KeyError):
            s.add_opening(999, OpeningKind.DOOR, offset_m=0.0,
                          width_m=0.9, head_m=2.0)
...
class TestRoomsAndSlabs:
    ...
    def test_stairs_stub_is_data_only(self):
        ...
        s = b.add_storey()
        b.add_storey()
        stub = s.add_stairs(storey_to=1, anchor=(2.0, 2.0),
...
def _full_building() -> Building:
    b = Building(name="house", position=Vec3(-24.0, 10.0, 8.0),
                 rotation=Quat.from_axis_angle(Vec3.UP, math.radians(15.0)),
                 defaults=_defaults(), tags=["rural", "demo"])
    s0 = b.add_storey()
    w_s = s0.add_wall((0, 0), (8, 0))
    s0.add_wall((8, 0), (8, 6), bulge=0.4, thickness_m=0.25)
    s0.add_wall((8, 6), (0, 6))
    s0.add_wall((0, 6), (0, 0))
    s0.add_opening(w_s.id, OpeningKind.DOOR, offset_m=3.5, width_m=0.9,
                   head_m=2.0)
    s0.add_opening(w_s.id, OpeningKind.WINDOW, offset_m=1.0, width_m=1.2,
                   sill_m=1.0, head_m=2.2)
    s0.add_room([(0, 0), (8, 0), (8, 6), (0, 6)], tag="living",
                meta={"style": "rustic"})
    s0.add_stairs(storey_to=1, anchor=(6.0, 5.0), direction_rad=math.pi / 2,
                  width_m=1.0)
...
class TestWorldAabb:
    ...
    def test_yaw_90_swaps_extents(self):
        b = Building(name="t", position=Vec3(0, 0, 0),
