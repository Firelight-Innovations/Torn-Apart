...
class GnarledOakDef(TreeSpeciesDef):
    ...
    def grow(self, rng: np.random.Generator,
        ...
        trunk = sb.trunk(height_m=5.5 + float(rng.uniform(-1.0, 1.5)),
                         base_radius_m=0.28, tip_radius_m=0.07,
                         segments=4, wobble_m=0.35,
                         lean_rad=_D(float(rng.uniform(0.0, 6.0))))
        limbs = sb.branches(trunk, count=(1, 2), t_range=(0.35, 0.95),
                            ...
                            radius_ratio=0.5,
                            upturn_rad=_D(18), segments=2)
        twigs = sb.branches(limbs, count=(1, 2),
                            pitch_set=(_D(85),),
