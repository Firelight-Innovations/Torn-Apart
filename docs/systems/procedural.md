...
- Auto-registration of built-in content at import time (textures: `"wasteland_ground"`, `"night_sky"`, `"rain_streak"`, `"grass_ground"`, `"dirt_ground"`, `"moon_surface"`, `"grass_tuft"`, `"dust_mote"`, `"leaf_sprite"`, `"flower_sprite"`; tree species: `"tree_gnarled_oak"`, `"tree_dead"`, `"bush_scrub"`, `"bush_berry"`).
...
| `LeafClusters`, `leaf_clusters_at_tips(sk, ids, rng, radius_m, per_tip, ...)` | `flora/leaves.py` | Foliage cluster centres/radii on branch-tip segments. |
| `TreeMesh`, `mesh_branches(sk, sides=4, uv_rect, tint)`, `mesh_leaf_clusters(clusters, rng, quads=3, ...)`, `merge_parts(*parts)` | `flora/mesher.py` | Tapered square prisms + crossed leaf quads → V3N3T2C4 arrays (`geometry_bridge.to_geom`-compatible).  **`colors[:,3]` is the per-vertex sway weight**, not alpha. |
| `AtlasLayout`, `bark_texture`, `leaf_texture`, `compose_atlas` | `flora/atlas.py` | 64×64 species atlas: left half tileable bark (opaque), right half binary-alpha leaf cutout; rng-consuming helpers, palette-driven. |
| `rasterize_impostor(sk, clusters, ...)`, `impostor_atlas(cells)` | `flora/impostor.py` | Headless software-rasterised far-LOD billboard cells (XZ orthographic; deterministic, pytest-testable). |
