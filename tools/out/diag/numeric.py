...
def sample(casc, wp, tex_arr):
    ...
    om=casc.window.world_origin_m; cm=casc.cell_m; n=casc.cells
    uv=[(wp[i]-om[i])/(cm*n) for i in range(3)]
    if not all(0<=u<1 for u in uv): return None
...
def report(tag):
    ...
    sun=pipe._last_sun
    sun_dir=np.array(sun[0]); sun_rad=np.array(sun[1]); amb=np.array(sun[4])
    ...
    for dist in [0.0, 30.0, 120.0, 250.0, 400.0]:
        ...
        for ci,c in enumerate(pipe.cascades):
            om=c.window.world_origin_m;cm=c.cell_m;n=c.cells
            uv=[(wp[i]-om[i])/(cm*n) for i in range(3)]
            pad=[0.02,0.01,0.005][ci]
            if all(pad<u<1-pad for u in uv):
                v=sample(c,wp,vis_t[ci]); r=sample(c,wp,rad_t[ci])
                chosen=(ci, v, r); break
        if chosen is None:
            # fallback
            rad=amb*0.6; vis=np.array([1,1,1.0]); src="FALLBACK"
            sunvis=1.0
        else:
            ci,v,r=chosen
        ...
        print(f"  dist={dist:4.0f} src={src:8s} sunvis={float(sunvis):.2f} rad={rad.round(3)} hdr={hdr.round(3)} ldr~={ldr.round(2)}")

# spawn
for _ in range(150): app.taskMgr.step(); app._clock.game_time_of_day=hold
report("SPAWN")
# fly forward 500m
cam=app.camera_go.transform; y=float(cam.position.y)
for i in range(500):
    y+=1.0; cam.position=Vec3(0.0,y,10.0); app.taskMgr.step(); app._clock.game_time_of_day=hold
report("MOVED +500m")
