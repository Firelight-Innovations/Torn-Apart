...
// --- radiance cascades (GpuLightingPipeline surface contract; inherited from
//     terrain_root) — copied verbatim from grass.frag so leaves are lit by the
//     SAME cascades as the grass and the ground they fall on -----------------
...
// --- froxel fog (same one-tap composite as grass / terrain) -----------------
...
void main() {
    ...
    // Light at the leaf centre, snapped to the terrain light-pixel grid so a
    // leaf and the ground share light patches (grass.frag idiom).
    ...
    frag_color = vec4(pow(ldr, vec3(1.0 / 2.2)), albedo.a);
}
