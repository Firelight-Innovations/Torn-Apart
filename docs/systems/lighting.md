...
- **GPU passes** (GLSL 430 compute sources in `lighting/shaders/*.comp` — `inject`, `propagate`, `shift`, `fog_scatter`, `fog_integrate` — loaded by `glsl.py` via `core.shader_source.load_glsl` and re-exported as `INJECT_COMPUTE`, `SHIFT_COMPUTE` etc.; `glsl.py` still owns the non-GLSL `MAX_LIGHTS`. Dispatched by `GpuLightingPipeline` in `gpu.py` via `GraphicsEngine.dispatch_compute`):
  1. **INJECT** (on change only) — per cell: occupancy raymarch toward the sun/moon (→ the `u_vis` visibility volume = all voxel shadows), straight-up march (→ skylight from `SkyState.sky_ambient`), first-bounce sunlight off solid neighbours tinted by their albedo, emissive-neighbour leak, and dynamic point/area lights with windowed inverse-square falloff + shadow march.  Two outputs: `u_direct` (the flood-fill forcing = skylight + **GI terms × `u_gi_gain`** + dynamic lights) and **`u_bounce_direct`** (the *un-gained* localized sources — first bounce + emissive leak + dynamic lights, no skylight) which the surface shader samples directly for crisp contact GI.  `u_gi_gain = 0.6/(1−decay)` compensates the contractive propagation's squash of localized sources (see gotcha 14); it scales the *forcing*, not the diffusion operator, so boundedness is untouched.
...
- **Surface contract** — `GpuLightingPipeline.bind_surface_inputs(node)` / `update_surface_inputs(node, sky_state)` feed `world/terrain_shader.py`, which samples the cascades (quantised to `light_quant_m` = 0.0625 m → 8×8×8 visible "light pixels" per voxel — the snap grid only; the GI *data* resolution is the cascade-0 cell), reads `u_vis` for celestial shadows, derives AO from occupancy, adds emission, samples `u_c0_bounce`/`u_c1_bounce` (the cascades' `bounce_direct` textures) for full-strength contact GI, and composites the froxel fog at its own depth.  `update_surface_inputs` also binds the dynamic-occluder AABBs (`u_num_boxes`/`u_box_min`/`u_box_max`, zero-filled to `MAX_OCCLUDERS` when empty — Panda3D asserts on any unbound GLSL uniform array) for the refinement march below.
  - **Per-fragment shadow refinement** (`terrain.frag::refineVis`): the trilinear cascade sample smears a shadow edge over a full cell — 2 m at cascade 1, i.e. a 4×4-voxel soft box, even though the *computed* `u_vis` data is crisp at 0.5 m.  Whenever the sampled celestial visibility lands in the penumbra band (`vis ∈ (0.02, 0.98)`) and the light is up, the fragment re-resolves it with its own occupancy march from the **quantized light-pixel probe** through the c0 → c1 → c2 occupancy chain (28/24/12 single-cell steps, nearest-cell `occCell` taps) × the analytic `boxVis` AABB test.  Fully-lit and fully-shadowed pixels (the vast majority) skip it, so the cost is confined to shadow edges.  Result: rendered shadow resolution == computed resolution at any camera distance.
...
- Radiance/visibility volumes are `rgba16f`, GPU-resident only — the engine never reads them back (`tools/light_probe.py` extracts them via `extract_texture_data` for offline diagnostics only).  Each cascade also owns a `bounce_direct` rgba16f volume (the un-gained localized sources, written by INJECT, sampled by `terrain.frag` as `u_c0_bounce`/`u_c1_bounce`).
...
14. **The contractive flood fill SQUASHES localized sources — never feed GI into `u_direct` un-gained.**  `propagate.comp`'s fixed point passes *broad* fields (skylight fills every open cell) through at full strength, but an *isolated* source equilibrates at only ≈ `(1−decay)` of its injected value (decay c0 = `exp(−0.5/4)` ≈ 0.88 → a 3–8× squash).  This is exactly why the first-bounce GI was invisible on screen (~4 % of ambient; A/B image mean diff 0.7/255) while skylight looked correct.  The fix is two-fold and both halves matter: `u_gi_gain = 0.6/(1−decay)` pre-amplifies the GI terms (first bounce + emissive leak — NOT skylight, NOT dynamic lights) in the flood-fill forcing so *spread* GI converges near physical strength, and the surface shader adds the un-gained `u_bounce_direct` sample for *crisp* contact GI that diffusion would blur away.  Auto-exposure normalizes broad ambient changes, so on-screen A/B stays subtle in bright scenes even when the radiance data moves +30–40 % — verify GI changes with `tools/light_probe.py` radiance readback, not by eyeballing screenshots.

15. **The shadow refinement march MUST include the analytic `boxVis` test** (`terrain.frag::refineVis` mirrors `inject.comp::boxVis`).  Dynamic occluders (dev cubes, future props) are NOT voxelised into the cascade occupancy — their shadows exist only because INJECT's analytic AABB test cut `u_vis`.  A penumbra refinement that marches occupancy alone "refines" those penumbra pixels to fully lit and **erases the object's shadow**.  Same trap for any future shadow re-resolve: every occlusion field that feeds `u_vis` must also feed the refinement.
