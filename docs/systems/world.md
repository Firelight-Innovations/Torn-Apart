...
18. **The grass GLSL hash chain has a python twin.** `lowbias32` + the h0–h4 chain in `world/grass_shaders.py` is mirrored line-for-line by `zones/grass_placement.py::instance_attribs` (which the headless tests pin).  Edit both or neither — a desync makes the tests describe blades that aren't where the GPU draws them.
