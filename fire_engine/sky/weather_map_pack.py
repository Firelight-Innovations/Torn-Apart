...
twin of :func:`fire_engine.wind.pack_wind_field`: it turns the
``(cells, cells, 4)`` float32 raster that
:meth:`fire_engine.weather.WeatherMap.rasterize` produces into a **float16**
buffer in Panda3D's 2-D RAM layout, ready for a single
``Texture(F_rgba16).set_ram_image`` on the render thread
(:class:`fire_engine.world.weather_renderer.WeatherMapComponent`).
...
* **BGRA** channel order (the known engine quirk: ``F_rgba16`` data textures
  ...
  :data:`fire_engine.weather.MAP_CHANNELS`); after the BGRA swap the bytes are
...
>>> from fire_engine.weather import WeatherSystem, WeatherMap
...
def pack_weather_map(raster: np.ndarray) -> bytes:
    ...
    deliberate difference from :func:`fire_engine.wind.pack_wind_field`, whose
    ...
    raster : numpy.ndarray
        ...
        :meth:`fire_engine.weather.WeatherMap.rasterize`.  Channels are
        ``R=coverage, G=density, B=precip, A=fog`` (the
        :data:`fire_engine.weather.MAP_CHANNELS` order), all dimensionless
