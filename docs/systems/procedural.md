...
- Auto-registration of built-in content at import time (currently: `"wasteland_ground"`, `"night_sky"`, `"rain_streak"`, `"grass_ground"`, `"dirt_ground"`, `"moon_surface"`, `"grass_tuft"`, `"dust_mote"`, `"leaf_sprite"`, `"flower_sprite"`, `"bush_sprite"`, `"tree_sprite"`).
...
| `"bush_sprite"` | `procedural/textures/bush_sprite.py` | `(48,144,4) uint8` shrub **atlas** — one row of 3 condition variants (living green / dry olive / dead brown): seeded elliptical foliage lobes broken by `pixel_noise` holes, posterised 4-tone ramp, twig pixels at the base, berry specks on the living variant.  Binary alpha.  Params: `width`, `height`. |
| `"tree_sprite"` | `procedural/textures/tree_sprite.py` | `(96,192,4) uint8` whole-tree **atlas** — one row of 3 condition variants (full green / autumn ochre / scraggly half-dead), 64×96 cells (2:3 — the renderer's quads keep this aspect): tapered bark-striated trunk + branch stubs under a lobed, noise-broken canopy with a posterised 5-tone ramp.  An anchor lobe always sits on the crown so the canopy connects to the trunk.  Binary alpha; trunk bases on the bottom row.  Params: `width`, `height`. |
