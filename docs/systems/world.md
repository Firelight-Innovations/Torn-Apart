...
- **SceneVisualFactory** (`world/scene_visuals.py`): the Panda3D half of editor-authored scene loading (the headless half is `fire_engine/scene/runtime.py::SceneRuntime` — see DECISIONS.md 2026-06-12). `attach(go, kind, obj)` gives each placed object its in-game form: `cube`/`sphere` → 1 m primitives from `world/primitives.py` (`load_cube_model`, `build_sphere_geom`; `CUBE_MODEL_SCALE` fixes the stock rgbCube's -1..1 span), `light` → a `PointLight` on `app.lighting_pipeline` (warm torch defaults; skipped+logged on the CPU backend), `empty`/`spawn` → no visual. Every object registers as an F1-overlay selectable, and the per-frame sync task mirrors Transform → NodePath/light AND **writes gizmo edits back into the authored store**, so in-game F5 persists moved objects. `teardown()` is synchronous (rebuild/double-load must not leak NodePaths or lights).
