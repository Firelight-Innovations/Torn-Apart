...
def run(out_name: str = "weather_verify/thunder.png",
        tod_h: float = 13.0, pitch_deg: float = 10.0,
        settle_frames: int = 140) -> Path:
    import numpy as np
    import main as demo
    ...
    cell = StormCell(
        id="v:0", kind=CellKind.THUNDERSTORM, spawn_time=spawn_time,
        spawn_pos=spawn_pos, duration_s=duration, radius_m=1100.0,
        peak_intensity=1.0, drift_bias=(0.0, 0.0))
    ...
    for i, sx in enumerate((-40.0, 30.0, 110.0)):
        publish(LightningStrikeEvent(
            pos=(ppos[0] + sx, ppos[1] + 150.0, cam.z + 210.0),
            ground_pos=(ppos[0] + sx, ppos[1] + 150.0, 8.0),
            seed=20260613 + i, time_abs=abs_t, cell_id=1, intensity=1.0))
