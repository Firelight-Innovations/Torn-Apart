...
pipe=app.lighting_pipeline; pipe.fog_enabled=False; pipe.bind_surface_inputs(app.terrain_root)
cam=app.camera_go.transform
cam.local_rotation=Quat.from_axis_angle(Vec3.RIGHT,math.radians(-12.0)).normalized()  # shallow, see horizon/edge
y=float(cam.position.y)
# stand at y=470 (just inside +500 edge), look toward +Y edge
for i in range(int((470-y)/1.2)):
    y+=1.2; cam.position=Vec3(0.0,y,12.0); app.taskMgr.step(); app._clock.game_time_of_day=hold
cam.position=Vec3(0.0,470.0,12.0)
for _ in range(30): app.taskMgr.step(); app._clock.game_time_of_day=hold
