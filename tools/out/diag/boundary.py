...
def chunk_surface_outside_all(camy):
    ...
    for c in pipe.cascades:
        if c.window.origin_cell is None: boxes.append(None); continue
        om=c.window.world_origin_m; sz=c.window.size_m
        boxes.append((om,sz))
    n_out=0; n_tot=0
    for (cx,cyc,cz) in app.chunk_manager.chunks.keys():
        if cz!=0: continue
        # chunk center world xy, surface z~8
        wx=(cx+0.5)*cm_m; wy=(cyc+0.5)*cm_m; wz=8.0
        n_tot+=1
        inside=False
        for b in boxes:
...
for i in range(700):
    ...
    if i%40==0:
        ...
        c2=pipe.cascades[2]
        print(f"frame {i} cam_y={y:.0f} chunks={nt} surf_outside_all_cascades={no} c2_inflight={c2._assembly_inflight} c2_box_y=[{c2.window.world_origin_m[1]:.0f}..{c2.window.world_origin_m[1]+c2.window.size_m:.0f}]")
