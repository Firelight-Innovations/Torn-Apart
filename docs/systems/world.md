...
| `pp.final_quad / pp.manager` | The composite card + the FilterManager (for `renderQuadInto` in later phases). |
| `pp.insert_pass_before_composite(card)` | Seam for the bloom / lens-flare / god-ray passes. |
| `pp.update(lighting_pipeline)` | Per-frame refresh (no-op in the scaffold — auto-exposure is applied in the surface shaders so it scales bloom too; the composite needs nothing yet). |
