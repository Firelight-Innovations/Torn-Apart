...
from fire_engine.world.weather import Synoptic, WeatherSystem
...
- `sky/weather.py` is now a **compatibility shim** that re-exports
  `WeatherType`/`WeatherSystem`/`LocalWeather` from `fire_engine.world.weather`.  New
  code imports from `fire_engine.world.weather` directly.
...
- **M4 GPU contract**: `WeatherMap.rasterize` output is packed by
  `fire_engine.world.sky.pack_weather_map` (fp16 BGRA, row-major — no transpose) and
