...
- **SceneVisualFactory** (`world/scene_visuals.py`): the Panda3D half of editor-authored scene loading (the headless half is `fire_engine/scene/runtime.py::SceneRuntime` — see DECISIONS.md 2026-06-12). `attach(go, kind, obj)` is **component-driven** — it walks `obj["components"]` (the source of truth; `kind` is only a fallback for pre-component data): a `Mesh{primitive}` → a 1 m cube/sphere from `world/primitives.py` (`load_cube_model`, `build_sphere_geom`; `CUBE_MODEL_SCALE` fixes the stock rgbCube's -1..1 span), a `Light{color,intensity,radius}` → a `PointLight` on `app.lighting_pipeline` built from the component's **own params** (defaults sourced from `fire_engine/scene/components.py`, not hardcoded; skipped+logged on the CPU backend), `SpawnPoint`/empty/disabled → no visual. So an `empty` given a Light emits light, and a cube whose Mesh is removed becomes an invisible transform. Param **hot-reload inside a running session is a non-goal** — params are read once at `attach` time (a fresh load reflects edits); the per-frame sync task only mirrors Transform → NodePath/light AND **writes gizmo edits back into the authored store**, so in-game F5 persists moved objects. The F1-overlay pick half-extent keys off Mesh presence, not `kind`. `teardown()` is synchronous (rebuild/double-load must not leak NodePaths or lights).
- **primitives** (`world/primitives.py`): shared in-code cube/sphere geometry (`build_sphere_geom` moved here from `wind_debug.py`).
- **Profiler instrumentation + render-side bridges.** `App._frame_task` wraps each loop stage in a `core.profiler` scope (`Input`/`Clock`/`Update`/`ChunkStream`/`Lighting`/`PostProcess`/`EventDrain`/`CameraSync`) and calls `begin_frame`/`end_frame`; `ComponentRegistry.run_frame` adds per-component-type child scopes (`Update:<Type>` etc.). `render/profiler_overlay.py::ProfilerOverlay` is the F3 in-game HUD (frame-time graph + 1%/0.1% lows + hitch counter + top scopes); `render/profiler_bridge.py::PStatsBridge` mirrors scopes/counters into Panda3D `PStatCollector`s. Both are constructed by `App._setup_profiler` only when `profiler_enabled` (truly free otherwise) and are headless-`None` like the other panda3d bridges. **Full details + the AI-agent runbook: `docs/systems/profiler.md`.**
- **SkyRendererComponent** (`world/sky_renderer.py`) + GLSL sources (`world/sky_shaders.py`): the render half of the sky/weather system. The headless simulation lives in `fire_engine/world/sky/`; this component drives `sky_system.update()` each frame and draws the sky dome — now a **per-pixel physical single-scattering atmosphere** (Rayleigh+Mie, constants mirrored from `sky/atmosphere.py`) with a 2.5×-sized limb-darkened sun disc tinted by its own transmittance, a 2.5×-sized moon disc textured with the seeded procedural `"moon_surface"` texture under the dynamic phase terminator, `"night_sky"` galaxy + twinkle + shooting stars composited post-tonemap — plus volumetric raymarched clouds (rain moved to `RainRendererComponent`, M6).  With `external_lighting=False` (CPU backend) it also applies exponential fog and the global terrain colour-scale; with `external_lighting=True` (GPU backend) it leaves terrain shading entirely to the lighting pipeline and samples the froxel fog's far slice for the dome.
...
The render half of the sky/weather system (`None` when panda3d is absent).  The headless half is `fire_engine.world.sky.SkySystem` (Layer 1).

| Symbol | Description |
|---|---|
| `SkyRendererComponent(base, sky_system, terrain_root, clock=None)` | Add to a GameObject via `add_component`. `base` = the `App`; `sky_system` = `fire_engine.world.sky.SkySystem` (or any duck-typed object with the `SkyState` fields â€” see `tools/screenshot.py --stub-sky`); `terrain_root` receives fog + light scale; `clock` enables star rotation + the deterministic shooting-star schedule. |
...
### BuildingRendererComponent (`world/building_renderer.py`, shaders in `world/building_shaders.py`)

Draws every building owned by a `fire_engine.buildings.BuildingManager` (free-form floorplan structures — see `docs/systems/buildings.md`) as real lit geometry. **GPU lighting backend only** (logs + disables itself on the CPU backend).

| Symbol | Description |
|---|---|
| `BuildingRendererComponent(base, building_manager, lighting_pipeline, bus)` | Add via `add_component`; wired in main.py behind `[debug] debug_demo_building` (the manager + provider are always created; the demo house is only placed behind the flag). |

**Draw structure**: one `GeomNode` per building. The building is meshed ONCE (`buildings.meshing.mesh_building` → building-LOCAL `MeshArrays`) and uploaded via `geometry_bridge.to_geom_node`; the node carries the building's `position`/`rotation` as its transform (`node.set_pos` + `set_quat(LQuaternionf(w,x,y,z))`). Buildings are unique — **no instancing**. The albedo is the procedural `plaster_wall` texture (`p3d_Texture0`, flat fallback if unavailable). The component subscribes to `BuildingChangedEvent`: `"added"`/`"modified"` re-mesh that id's node, `"removed"` detaches it; all node work happens in `late_update` (events only mark dirty), and a `manager.version` jump with no pending events triggers a full resync.

**Local-space vs terrain (the key divergence — `building.vert` MUST honour it)**: terrain chunk meshes are emitted in **world space at the chunk origin**, so their node transform is identity and `terrain.vert` could read the raw vertex. Building meshes are emitted in **building-local space** and the world transform lives on the node, so `building.vert` derives `v_world = (p3d_ModelMatrix * p3d_Vertex).xyz` (and rotates the normal by `mat3(p3d_ModelMatrix)`). Get this wrong and `lit_surface.glsl` samples the cascades at the wrong world cell — the building lights as if it were sitting at the origin. The upside: moving or rotating a building is a transform write, never a remesh.

`building.frag` is the full **`lit_surface.glsl`** contract (see *Lighting any object* below): one `sampleCascades` at the fragment's quantised world position, soft-penumbra shadow refinement (`u_refine` pinned 1.0 on `building_root` — terrain-grade, walls deserve crisp edges), two-point voxel AO, fog, and the `u_hdr_output`-gated finish. Sampler budget **11** (9 cascade + 1 fog + 1 albedo). **Light occlusion is a v1 no-op**: buildings are *lit* but do not yet *shadow* the cascades — `BuildingOccupancyRasterizer` is registered with the pipeline (store-only) but returns without splatting (see `docs/systems/buildings.md` / `docs/systems/lighting.md`). Interiors are dim only via the openings until that lands.
...
**Budgets**: GL 3.3 guarantees only **16 fragment samplers**; the library uses 10 (9 cascade + 1 fog), terrain sits at 15 total, and `building.frag` at 11 (the 10 library samplers + 1 plaster albedo). Check `tests/test_lit_surface.py::TestSamplerBudget` before adding samplers. The `LIT_REFINE` block also adds 2×16 `vec4` occluder arrays — skip the define on shaders that draw tiny fragments (motes).
...
```python
q_yaw   = Quat.from_axis_angle(Vec3.UP,   yaw)    # render Z rotation
...
```python
from fire_engine.render import instantiate, ComponentRegistry
