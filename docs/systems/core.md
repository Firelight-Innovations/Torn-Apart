...
| `lod_max_rank` | int | 3 | Highest coarse LOD rank generated by `CoarseLodStreamer` (`world/terrain/lod/coarse_streamer.py`); `L3` = 8× downsample. `0` = near-only (P1 behaviour — no distant horizon). |
| `lod_near_radius_chunks` | int | 6 | Rank-0 (`L0`) editable/lit/saved Chebyshev radius in chunks (== `view_distance_chunks`); columns inside it stay native chunks, never coarse nodes. |
| `lod_far_radius_chunks` | int | 32 | Outer XY Chebyshev radius (chunks) of the coarse desired-node window — the distant-horizon extent (32 chunks = 512 m). |
| `lod_band_l1_m` | float | 32.0 | Distance (m) at/above which a column drops to coarse rank `L1` (2×). `searchsorted` band threshold in `desired_node_set`. |
| `lod_band_l2_m` | float | 96.0 | Distance (m) for the `L1`→`L2` (4×) coarse-rank transition. |
| `lod_band_l3_m` | float | 192.0 | Distance (m) for the `L2`→`L3` (8×) coarse-rank transition. |
| `lod_downsample_mode` | str | "any" | Material reduce for coarse cells: "any" = max-id (grass beats dirt on the skin), "majority" = vectorized mode. Solidity is always ANY. Deterministic — only affects coarse mesh output. |
| `lod_coarse_submit_per_frame` | int | 4 | Max coarse mesh jobs the `CoarseLodStreamer` submits per frame to its separate `TerrainLodPool` (coarsest-far-first); separate budget so near streaming is never starved. |
| `lod_coarse_uploads_per_frame` | int | 4 | Max coarse-node Geoms uploaded to the scene graph per frame (nearest-first); separate budget from `lod_max_uploads_per_frame`. |
